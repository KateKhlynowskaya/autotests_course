...
def everything_for_your_cat(cats_data):
    ...
    dkt = {}
    for i in cats_data:
        owner_name = '{first_name} {second_name}'.format(first_name=i[2], second_name=i[3])
        if not dkt.get(owner_name):
            dkt[owner_name] = []
        cat = '{cat_name}, {age}'.format(cat_name=i[0], age=i[1])
        dkt[owner_name].append(cat)
    for owner_name in dkt:
        our_str += '{owner_name}: {cats}\n'.format(owner_name=owner_name, cats='; '.join(dkt[owner_name]))
    return our_str
