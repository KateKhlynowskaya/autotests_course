    ...
    def new_salary(self):
        ...
        for a in ''.join(self.subdivisions):
            if a in array_dict:
                ...
                array_dict[a] = 1
        sorted_dict = dict(sorted(array_dict.items(), key=lambda item: item[1], reverse=True))
        subdivisions_words_sum = 0
        for index, key in enumerate(sorted_dict):
            subdivisions_words_sum += sorted_dict[key]
            if index == 2:
                break

        return 1337 * self.age * subdivisions_words_sum
